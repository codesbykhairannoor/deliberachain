...
export default function AdminPortal() {
  ...
  const handleTakedown = async (id: bigint) => {
    if (!confirm(t.takedownDesc)) return;
  ...
  if (roleLoading) return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center text-foreground p-6">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-60">{t.verifyingInstitutional}</p>
    </div>
  );
        ...
        <main className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-foreground relative overflow-hidden">
            ...
            >
                ...
                <div className="relative z-10">
                    ...
                    <h1 className="text-3xl font-black text-foreground mb-4 uppercase tracking-tighter leading-none">{t.platformModeration}</h1>
                    <p className="text-muted-foreground text-sm font-medium mb-12 italic">{t.lockedMsg}</p>
                    ...
                    <div className="space-y-6">
                        ...
                        >
                            {lang === 'id' ? 'Buka Akses' : 'Unlock Protocol'}
                        </button>
                    </div>
                </div>
            </motion.div>
            ...
            <div className="lg:col-span-4 space-y-8">
                <div className="bg-muted/50 border border-border p-12 rounded-[3.5rem] shadow-inner relative overflow-hidden group">
                    ...
                    <h3 className="text-xs font-black text-foreground mb-10 flex items-center gap-4 uppercase tracking-[0.3em] italic">
                        <Terminal size={20} className="text-blue-500" /> {t.protocolNode}
                    </h3>
                    <div className="space-y-4">
                        <div className="flex justify-between items-center bg-background p-4 rounded-xl border border-border shadow-sm">
                            <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{t.netNetwork}</span>
                            <span className="text-[10px] font-black text-green-500 uppercase tracking-widest">Base Sepolia</span>
                        </div>
                        <div className="flex justify-between items-center bg-background p-4 rounded-xl border border-border shadow-sm">
                            <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{t.netHeight}</span>
                            <span className="text-[10px] font-black text-vault-amber uppercase tracking-widest">#{blockNumber?.toString() || "Syncing..."}</span>
                        </div>
                        <div className="flex justify-between items-center bg-background p-4 rounded-xl border border-border shadow-sm">
                            <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{t.netDatabase}</span>
                            <span className="text-[10px] font-black text-foreground uppercase tracking-widest">{allAspirations?.length || 0} {t.netRecords}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-10 bg-red-600/5 border border-red-600/10 rounded-[3rem] group">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.4em] text-red-600 italic">{t.securityProtocol}</span>
                    </div>
            ...
            <div className="lg:col-span-8 bg-muted/30 border border-border p-12 rounded-[4rem] shadow-inner">
                <div className="flex items-center justify-between mb-12">
                    <h3 className="text-2xl font-black text-foreground flex items-center gap-4 uppercase tracking-tighter italic">
                        <Activity className="text-vault-amber" /> {t.moderationQueue}
                    </h3>
                ...
                <div className="max-h-[800px] overflow-y-auto pr-4 custom-scrollbar">
                    {isAspLoading ? (
                      <div className="flex flex-col items-center justify-center p-20 gap-6">
                          ...
                          <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-40">{t.syncingQueue}</p>
                      </div>
                      ...
                      <div className="text-center p-20 border-4 border-dashed border-border rounded-[3rem] group hover:border-vault-amber/30 transition-colors">
                          ...
                          <p className="text-muted-foreground font-black uppercase tracking-widest text-xs">{t.noRecordsFound}</p>
                      </div>
                    ) : (
                      <div className="space-y-6">
                             ...
                             >
                                 <div className="flex-1 pr-10">
                                     <div className="flex items-center gap-4 mb-3">
                                        ...
                                        {asp.status === 1 ? (
                                            <span className="px-4 py-1.5 rounded-full text-[8px] font-black tracking-[0.2em] bg-red-600 text-white shadow-xl uppercase">{t.statusHidden}</span>
                                        ) : asp.status === 2 ? (
                                            <span className="px-4 py-1.5 rounded-full text-[8px] font-black tracking-[0.2em] bg-green-600 text-white shadow-xl uppercase">{t.statusResolved}</span>
                                        ) : (
                                            <span className="px-4 py-1.5 rounded-full text-[8px] font-black tracking-[0.2em] bg-blue-600 text-white shadow-xl uppercase">{t.statusActive}</span>
                                        )}
                                     </div>
