...
export default function AdminPortal() {
  ...
  if (roleLoading) return <div className="p-20 text-white">Verifying Access...</div>;
    ...
    return (
        <main className="min-h-screen bg-black flex flex-col items-center justify-center p-6">
            ...
            >
                ...
                <h1 className="text-2xl font-black text-white mb-2 uppercase tracking-tighter">Content Moderation</h1>
                <p className="text-slate-500 text-sm mb-8">Masukkan kode otorisasi untuk mengakses kontrol Takedown.</p>
                ...
                <input 
                    ...
                    className="w-full bg-black/50 border border-white/10 p-4 rounded-xl text-center text-white focus:border-red-500 outline-none mb-4"
                />
                <button 
                    onClick={handleLogin}
                    className="w-full bg-red-600/20 border border-red-600/50 hover:bg-red-600 hover:text-white text-red-500 font-bold py-4 rounded-xl transition-all"
                >
                    Unlock System
                </button>
            </motion.div>
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <header className="flex justify-between items-center mb-12">
           <div>
              <h1 className="text-4xl font-black text-white tracking-tight uppercase">Platform <span className="text-red-600">Moderation</span></h1>
              <p className="text-slate-500 text-sm">Hide & Takedown Control Panel</p>
           </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-4 space-y-6">
                <div className="bg-vault-card border border-white/5 p-8 rounded-3xl">
                    ...
                    <div className="flex justify-between items-center text-xs text-slate-400 font-mono mt-2 pt-2 border-t border-white/5">
                        <span>Total Records</span>
                        <span className="text-white">{allAspirations?.length || 0}</span>
                    </div>
                </div>
            ...
            <div className="lg:col-span-8 bg-vault-card border border-white/5 p-8 rounded-3xl">
                ...
                {isAspLoading ? (
                  ...
                  <div className="text-center p-10 text-slate-500">No aspirations found on-chain.</div>
                ) : (
                         ...
                         <div key={i} className="flex justify-between items-center p-4 bg-black/40 border border-white/5 rounded-2xl">
                             <div>
                                 <div className="flex items-center gap-2 mb-1">
                                    <span className="text-xs text-slate-500 font-mono">ID: {asp.id.toString()}</span>
                                    {asp.status === 1 ? (
                                 ...
                                 <h4 className="text-white font-bold">{asp.title}</h4>
                                 <p className="text-xs text-slate-400 mt-1">{asp.category}</p>
                             </div>
