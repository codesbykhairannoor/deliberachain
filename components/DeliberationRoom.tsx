...
export default function DeliberationRoom({ client, contract }: DeliberationRoomProps) {
  ...
  if (isLoading) return <div className="p-10 text-white animate-pulse">Loading Room...</div>;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm">Topik yang sedang hangat dibahas masyarakat.</p>
        </div>
        ...
        {trending.map((item, i) => (
          ...
          >
            ...
            <div className="flex-1">
               <h4 className="font-bold text-white group-hover:text-vault-amber transition-colors line-clamp-1">{item.title}</h4>
               <div className="flex items-center gap-4 mt-2">
                  <span className="text-[10px] text-slate-500 flex items-center gap-1 font-mono uppercase bg-white/5 px-2 py-0.5 rounded">
            ...
            <div className="hidden md:block">
               <div className="flex -space-x-2">
                  {[1,2,3].map(j => (
                    <div key={j} className="w-8 h-8 rounded-full border-2 border-black bg-zinc-800 flex items-center justify-center text-[10px] text-white">
