...
export default function CriticismSection({ client, contract }: CriticismSectionProps) {
  ...
  if (isLoading) return <div className="p-10 text-foreground animate-pulse">Loading Feedback...</div>;
  ...
  return (
    <div className="space-y-6">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground text-sm">Suara kritis masyarakat untuk perbaikan kebijakan.</p>
      </div>
        ...
        {critiques.map((item, i) => (
          ...
          >
             ...
             <div className="flex items-center gap-4 border-t border-white/5 pt-4">
                <button className="flex items-center gap-1 text-muted-foreground hover:text-red-500 transition-colors text-xs font-bold">
                    <ThumbsDown size={14} /> Dukung Kritik ({10 + i * 5})
                </button>
                <button className="flex items-center gap-1 text-muted-foreground hover:text-foreground transition-colors text-xs font-bold">
